...
class DeclListNode extends ASTnode {
    ...
    public int genOff(int off) {
        // calculates the total space we need
...
class FormalsListNode extends ASTnode {
    ...
    public FormalsListNode(List<FormalDeclNode> S) {
        myFormals = S;
        formOff = 0; // to calculate the total formal offset
    }
    ...
    public List<Type> nameAnalysis(SymTable symTab) {
        ...
        for (FormalDeclNode node : myFormals) {
            ...
            if (sym != null) {
                ...
                sym.offset = formOff;
                formOff -= 4; // we are assuming that all the variables are of size 4
            }
        }
        return typeList;
    }    
    ...
    public void codeGen(PrintWriter p) {
        Codegen.p = p;
        int off = 8;
        int add = 4 * myFormals.size();
        off += add;
        Codegen.generate("addu", Codegen.FP, Codegen.SP, Integer.toString(off));
       }

    public int getFormOff() {
        return formOff;
    }
...
class FnBodyNode extends ASTnode {
     ...
     */
    public void nameAnalysis(SymTable symTab, int formals) {
        int off = formals - 8; // 8 bytes for return address + control link


        myDeclList.nameAnalysis(symTab);
        // generate offset for locals
        off = myDeclList.genOff(off);
        myStmtList.nameAnalysis(symTab);
        // no need to generate an offset over here
        off = myStmtList.genOff(off);
        // offset and formals are negative
        locals = formals - off - 8;
    }    
 
    public void codeGen(PrintWriter p, String lab) {
        Codegen.p = p;
        if (locals <= 0) {
            // no locals
        }
    ...
    private int locals; // space for locals
}
	...
	public void codeGen(PrintWriter p, String lab) {
        ...
        for (StmtNode node : myStmts) {
           if (node instanceof ReturnStmtNode) {
		       ((ReturnStmtNode)node).codeGen(p, lab);
		   }
		   else
			   node.codeGen(p);
        }
...
abstract class DeclNode extends ASTnode {
    ...
    public void typeCheck() { }
    public int genOff(int off) {return off;} 
}
...
class VarDeclNode extends DeclNode {
    ...
    public int genOff(int off) {
        return off - 4; // since a variable will only have a size of 4 bytes
    }

    public void codeGen(PrintWriter p){
	Sym sym = myId.sym();
	if(sym.isAccess()) { // if it is global
	    p.println("\t.data");
	    p.println("\t\t.align 2");
    ...
    public Sym nameAnalysis(SymTable symTab, SymTable globalTab) {
        ...
        // after we insert, let's check if the symbol is global or not
        if (symTab.getSize() == 1) // it is global 
            sym.access = true;
...
class FnDeclNode extends DeclNode {
	...
	public void codeGen(PrintWriter p) {
		String Lab = "_" + myId.name() + "_Exit";
		Codegen.p = p;
		int RAOff = ((FnSym)myId.sym()).formals;
		int FPOff = RAOff - 4;
		// preamble
		...
		myBody.codeGen(p, Lab); 
		Codegen.genLabel(Lab);
		Codegen.generateIndexed("lw", Codegen.RA, Codegen.FP, RAOff);
		Codegen.generate("move", Codegen.T0, Codegen.FP);
		Codegen.generateIndexed("lw", Codegen.FP, Codegen.FP, FPOff);
    ...
    public Sym nameAnalysis(SymTable symTab) {
        ...
        if (sym != null) {
            ...
            sym.formals = myFormalsList.getFormOff();
        }
        // also pass the offset of the formals
        myBody.nameAnalysis(symTab, sym.formals); // process the function body
...
class PostIncStmtNode extends StmtNode {
    ...
    public void codeGen(PrintWriter p) {
        ...
        if(sym.isAccess()) { // global
           Codegen.generate("sw", Codegen.T0 , "_" + ((IdNode)myExp).name());
        }
        else {
            Codegen.generateIndexed("sw", Codegen.T0, Codegen.FP, (sym.offset));
        }
    }
...
class WriteStmtNode extends StmtNode {
	...
	public void codeGen(PrintWriter p) {
		...
		} else if(globalType instanceof StringType){
			Codegen.generate("li", Codegen.V0, 4);
		} else if(globalType instanceof BoolType){
			Codegen.generate("li", Codegen.V0, 1);
		} else{
			// not sure
		}
...
class IfStmtNode extends StmtNode {
 ...
 public void codeGen(PrintWriter p){
	...
	myExp.codeGen(p);
	Codegen.genPop(Codegen.T0);
	Codegen.generate("beq", Codegen.T0, Codegen.FALSE, lab);
	myStmtList.codeGen(p, lab);
	Codegen.genLabel(lab);	
    }	
...
class IfElseStmtNode extends StmtNode {
    ...
    public int genOff(int off) {
        off = myThenDeclList.genOff(off);
        off = myThenStmtList.genOff(off);
        off = myElseDeclList.genOff(off);
        off = myElseStmtList.genOff(off);
        return off;
    }
	public void codeGen(PrintWriter p) {
		Codegen.p = p;
		myExp.codeGen(p);
		String lab1 = Codegen.nextLabel();
		String lab2 = Codegen.nextLabel();
		Codegen.genPop(Codegen.T0);
		Codegen.generate("beq", Codegen.T0, Codegen.FALSE, lab1);
		myThenStmtList.codeGen(p, lab1);
		Codegen.generate("b", lab2);
		Codegen.genLabel(lab1);
		myElseStmtList.codeGen(p, lab1);
		Codegen.genLabel(lab2);
    }
...
class WhileStmtNode extends StmtNode {
	...
	public void codeGen(PrintWriter p){
		...
		myExp.codeGen(p);
		Codegen.genPop(Codegen.T0);
		Codegen.generate("beq", Codegen.T0, "0", Lab1);
		myStmtList.codeGen(p, Lab1);
		Codegen.generate("b", Lab2);
		Codegen.genLabel(Lab1);
    }
...
class StringLitNode extends ExpNode {
    ...
    public void codeGen(PrintWriter p) {
        Codegen.p = p;
        p.println("\t.data");
        String genLab = Codegen.nextLabel();
        p.println(genLab + ": .asciiz " +"\"" + myStrVal + "\"");
        p.println(".text");
        Codegen.generate("la", Codegen.T0, genLab);
        Codegen.genPush(Codegen.T0);
    }
    ...
    public void codeGen(PrintWriter p) {
        ...
        if (this instanceof NotNode) {
            ...
            // the following expression:
                // sets $T0 to 1, if it was initially 0
                // or   $T0 to 0, if it was initially 1
...
abstract class BinaryExpNode extends ExpNode {
    ...
    public void codeGen (PrintWriter p) {
        ...
        if (this instanceof AndNode) {
            ...
            myExp1.codeGen(p);
            Codegen.genPop(Codegen.T0);
            String finLabel = Codegen.nextLabel();
            // if false then branch to the next part
            Codegen.generate("beq", Codegen.T0, Codegen.FALSE, finLabel);
            

            // if the control comes here then exp1 was true
            // seeing if the right operand evaluates to true
            myExp2.codeGen(p);
            Codegen.genPop(Codegen.T0);
            Codegen.genLabel(finLabel, "Finally, the end of AND");
            Codegen.genPush(Codegen.T0);
        }
            ...
            Codegen.genPop(Codegen.T0);
            String finLabel = Codegen.nextLabel();
            // if true then branch to the next part
            Codegen.generate("beq", Codegen.T0, Codegen.TRUE, finLabel);
            ...
            myExp2.codeGen(p);
            Codegen.genPop(Codegen.T0);
            Codegen.genLabel(finLabel, "Finally, the end of OR");
            Codegen.genPush(Codegen.T0);
        }
        else {
            // String comparison
            if (myExp1 instanceof StringLitNode 
                && myExp2 instanceof StringLitNode) {
                String str1 = ((StringLitNode)myExp1).getMyStrVal();
                String str2 = ((StringLitNode)myExp2).getMyStrVal();
                if (str1.equals(str2)) {
                   Codegen.generate("li", Codegen.T0, 0);
                   Codegen.genPush(Codegen.T0);
                   Codegen.generate("li", Codegen.T1, 0); 
                   Codegen.genPush(Codegen.T1);
                   // both T1 and T0 have same values
                }
                else {
                   Codegen.generate("li", Codegen.T0, 0);
                   Codegen.genPush(Codegen.T0);
                   ...
                   Codegen.genPush(Codegen.T1);
                   // both T1 and T0 have different values
                }
            }
            else 
            {
            ...
            Codegen.genPop(Codegen.T1);
            Codegen.genPop(Codegen.T0);
            Codegen.generate(opt, Codegen.T0, Codegen.T1, Codegen.T1); 
            Codegen.genPush(Codegen.T0);
        }
    }
